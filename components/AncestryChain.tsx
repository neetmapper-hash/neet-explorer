...
function parseConceptId(id: string): { classNum: number; chapterNum: number } {
  try {
    // Format: physics_c11_ch04_s02  or  bio_c12_ch03_m01
  ...
  const fetchQuestions = async (batchNum: number) => {
    ...
    try {
      const res = await fetch('/api/test-questions', {
        ...
        body: JSON.stringify({
          concept_name: concept.concept_name,   // was concept_title
          summary: concept.summary,             // was description
...
function ExplainPopup({ concept, classNum, chapterNum, onClose }: ExplainPopupProps) {
  ...
  const openInNewTab = () => {
    const content = `<html><head><title>${concept.concept_name} — NEET Explorer</title>
      ...
      <h2>NCERT Summary</h2><div class="desc">${concept.summary}</div>
      ...
      >
        ...
        <div className="flex items-start justify-between px-5 py-4 border-b flex-shrink-0" style={{ borderColor: '#1e1e1e' }}>
          <div>
            ...
            <div className="text-white font-semibold text-base">{concept.concept_name}</div>
          </div>
        ...
        <div className="overflow-y-auto flex-1 px-5 py-4">
          {mode === 'explain' ? (
            <>
              ...
              {/* Formula display */}
              {concept.formula && concept.formula.length > 0 && (
                <div className="mt-4">
                  <div className="text-[10px] font-mono text-[#444] uppercase tracking-widest mb-2">Formula</div>
                  <div className="flex flex-col gap-1">
                    {concept.formula.map((f, i) => (
                      <code key={i} className="text-xs px-3 py-2 rounded-lg" style={{ background: '#1a1a2e', color: '#93c5fd', fontFamily: 'monospace' }}>
                        {f}
                      </code>
                    ))}
                  </div>
            ...
            return (
              <div key={concept.id}>
                <div className="rounded-xl p-4 border-l-4" style={{ borderColor: cfg.color, background: '#1a1a1a' }}>
                  ...
                  <div className="text-white font-medium text-sm mb-1">{concept.concept_name}</div>
                  <div className="text-[#aaa] text-sm leading-relaxed">{concept.summary}</div>
                  {concept.key_terms && concept.key_terms.length > 0 && (
                  ...
                  {concept.domain && (
                    <div className="mt-2">
                      <span className="text-[9px] px-2 py-0.5 rounded-full font-mono" style={{ background: '#1e1e2e', color: '#7878ff' }}>
                        {concept.domain}
                      </span>
                    </div>
                  )}
          ...
          {chain.map((concept, i) => {
            ...
            return (
              <div key={concept.id} className="flex items-center gap-3 px-3 py-2 rounded-lg bg-[#1a1a1a] text-sm">
                ...
                <span className="text-[#ccc]">{concept.concept_name}</span>
              </div>
            );
          })}
        </div>
