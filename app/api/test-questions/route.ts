...
export async function POST(req: NextRequest) {
  try {
    const { concept_name, summary, key_terms, class: classNum, chapter, batch } = await req.json();
    const concept_title = concept_name;  // field renamed in new JSON schema
    const description = summary;

    if (!concept_name) {
      return NextResponse.json({ error: 'concept_name required' }, { status: 400 });
    }
